...
教程地址:https://www.ucloud.cn/yun/121703.html
...
```bash
bash <(curl -L -s check.unlock.media)bash <(curl -L -s check.unlock.media) -M 4 # 只检测ipv4结果bash <(curl -L -s check.unlock.media) -M 6 # 只检测ipv6结果
```
